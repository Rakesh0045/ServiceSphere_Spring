...
const ProviderDashboard = () => {
    ...
    const stats = useMemo(() => ({
        ...
        totalEarnings: bookings.filter(b => b.status === "Completed")
            .reduce((acc, s) => acc + (parseFloat(s.price) || 0), 0).toFixed(2),
    }), [services, bookings]);
            ...
            <main className="content-area">
                ...
                <div className="tabs">
                    ...
                    <button className={`tab-btn ${activeTab === 'bookings' ? 'active' : ''}`} onClick={() => setActiveTab('bookings')}>
                        ...
                        {bookings.filter(b => b.status === 'Pending').length > 0 && (
                            <span style={{ background: 'var(--warning-color)', color: 'white', borderRadius: '50%', padding: '0.25em 0.6em', marginLeft: '0.5em', fontSize: '0.85em', fontWeight: 700, verticalAlign: 'middle' }}>
                ...
                {activeTab === 'addService' && (
                    <section className="content-panel">
                        ...
                        <form className="add-service-form" onSubmit={handleSubmit}>
                            ...
                            <div className="form-group full-width"><label>Description</label><textarea className="form-textarea" placeholder="Describe the service you offer..." value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} /></div>
                            <div className="form-group"><label>Category</label><select required className="form-select" value={form.category} onChange={e => setForm({ ...form, category: e.target.value })}><option value="">Select Category *</option>{FALLBACK_CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}</select></div>
                            <div className="form-group"><label>Price (₹)</label><input className="form-input" placeholder="e.g., 500" type="number" value={form.price} onChange={e => setForm({ ...form, price: e.target.value })} /></div>
                            <div className="form-group"><label>Location</label><input className="form-input" placeholder="e.g., Bhubaneswar" value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} /></div>
                ...
                {activeTab === 'bookings' && (
                    <section className="content-panel">
                        ...
                        {bookings.filter(b => b.status === 'Pending').length > 0 && (
                            <div style={{ background: 'rgba(243,156,18,0.15)', color: 'var(--warning-color)', borderRadius: '8px', padding: '1rem', marginBottom: '1.5rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <div className="modal-content" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleUpdateService}>
                            <div className="modal-form-grid">
                                ...
                                <div className="form-group"><label>Price (₹)</label><input type="number" className="form-input" value={editingService.price || ''} onChange={e => setEditingService({ ...editingService, price: e.target.value })} /></div>
                                ...
                                <div className="form-group"><label>Location</label><input type="text" className="form-input" value={editingService.location || ''} onChange={e => setEditingService({ ...editingService, location: e.target.value })} /></div>
            ...
            {isDeleteModalOpen && serviceToDelete && (
                <div className="modal-overlay" onClick={() => setIsDeleteModalOpen(false)}>
                    <div className="modal-content confirm-delete-modal" onClick={e => e.stopPropagation()}>
                        ...
                        <p>Are you sure you want to delete "{serviceToDelete?.service_name}"? This cannot be undone.</p>
                        <div className="modal-actions">
